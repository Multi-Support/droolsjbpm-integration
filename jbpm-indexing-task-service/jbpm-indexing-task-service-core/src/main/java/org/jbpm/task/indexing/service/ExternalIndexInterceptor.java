...
public class ExternalIndexInterceptor extends AbstractInterceptor implements
        TaskLifeCycleEventListener, TaskContentReader {

    private final ExternalIndexService<?> service;

    private static final ThreadLocal<Map<Long, Task>> modifiedTasks =
            new ThreadLocal<Map<Long, Task>>() {
                protected Map<Long, Task> initialValue() {
                    return new HashMap<Long, Task>();
                };
            };
    private static final ThreadLocal<Map<Long, Task>> insertedTasks =
            new ThreadLocal<Map<Long, Task>>() {
                protected Map<Long, Task> initialValue() {
                    return new HashMap<Long, Task>();
                };
            };
    private static final ThreadLocal<Map<Long, Map<Long, Content>>> contents =
            new ThreadLocal<Map<Long, Map<Long, Content>>>() {
                protected Map<Long, Map<Long, Content>> initialValue() {
                    return new HashMap<Long, Map<Long, Content>>();
                };
            };
    ...
    public <T> T execute(final Command<T> command) {
        try {
            final ExternalIndexInterceptor me = this;
            T result = executeNext(new TaskCommand<T>() {
    ...
    protected void populateContentsFromCommand(Command<?> command,
        ...
        if (command instanceof AddContentCommand) {
            ...
            byte[] content =
                    ContentMarshallerHelper.marshallContent(
                        addTaskCommand.getParams(), null);
            Content contentObject = TaskModelProvider.getFactory().newContent();
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            try {
                ObjectOutputStream out = new ObjectOutputStream(baos);
                out.writeLong(addTaskCommand.getTask().getTaskData()
                        .getDocumentContentId());
                out.writeInt(content.length);
                out.write(content);
                out.flush();
                contentObject.readExternal(new ObjectInputStream(
            ...
            byte[] content =
                    ContentMarshallerHelper.marshallContent(
                        completeTaskCommand.getData(), null);
            Content contentObject = TaskModelProvider.getFactory().newContent();
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            try {
                ObjectOutputStream out = new ObjectOutputStream(baos);
                out.writeLong(0L); // TODO needs to have the task object to get
                                   // the output content id, not present in the
                                   // complete command.
                out.writeInt(content.length);
                out.write(content);
                out.flush();
            ...
            byte[] content =
                    ContentMarshallerHelper.marshallContent(
                        failTaskCommand.getData(), null);
            Content contentObject = TaskModelProvider.getFactory().newContent();
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            try {
                ObjectOutputStream out = new ObjectOutputStream(baos);
                out.writeLong(0L); // TODO needs to have the task object to get
                                   // the fault content id, not present in the
                                   // fail command.
                out.writeInt(content.length);
                out.write(content);
                out.flush();
            ...
            switch (spCommand.getProperty()) {
                case SetTaskPropertyCommand.FAULT_PROPERTY:
                    ...
                    Content contentObject1 =
                            TaskModelProvider.getFactory().newContent();
                    ByteArrayOutputStream baos1 = new ByteArrayOutputStream();
                    try {
                        ObjectOutputStream out = new ObjectOutputStream(baos1);
                        out.writeLong(0L); // TODO needs to have the task object
                                           // to get the fault content id, not
                                           // present in the fail command.
                        out.writeInt(content1.length);
                        out.write(content1);
                        out.flush();
                        contentObject1.readExternal(new ObjectInputStream(
                ...
                case SetTaskPropertyCommand.OUTPUT_PROPERTY:
                    ...
                    if (spCommand.getValue() instanceof Map) {
                        ...
                        try {
                            ...
                            out.writeLong(0L); // TODO needs to have the task
                                               // object to get the fault
                                               // content id, not present in the
                                               // fail command.
                            ContentData content2 =
                                    ContentMarshallerHelper.marshal(
                                        spCommand.getValue(), null);
                            out.writeInt(content2.getContent().length);
                            out.write(content2.getContent());
                            out.flush();
                            contentObject2.readExternal(new ObjectInputStream(
                                    new ByteArrayInputStream(baos2
                                            .toByteArray())));
                        } catch (Exception e) {
                            throw new RuntimeException(e);
                        }
                        ...
                        try {
                            ...
                            out.flush();
                            contentObject2.readExternal(new ObjectInputStream(
                                    new ByteArrayInputStream(baos3
                                            .toByteArray())));
                        } catch (Exception e) {
                            throw new RuntimeException(e);
                        }
